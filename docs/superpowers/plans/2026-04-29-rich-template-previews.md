# Rich Template Previews Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Extract rich dummy resume data into a shared constant so both the first-time user experience and template previews have complete data.

**Architecture:** We will create `dummyResumeData` in `src/lib/dummy-resume.ts` containing the arrays and objects from `initialResume`. Then, we spread this into `blankResumeState` and `initialResume` so they share the content while maintaining their own unique metadata (`id`, `name`, `templateId`).

**Tech Stack:** React, TypeScript, Vite, Vitest

---

### Task 1: Extract dummy data and update dummy-resume.ts

**Files:**
- Modify: `src/lib/dummy-resume.ts`

- [ ] **Step 1: Write the updated `src/lib/dummy-resume.ts` with `dummyResumeData`**

Replace the contents of `src/lib/dummy-resume.ts` with the extracted data and the updated `blankResumeState`.
...
git commit -m "refactor: extract dummyResumeData and apply to blankResumeState"
```

### Task 2: Refactor resume-store.ts to use dummyResumeData

**Files:**
- Modify: `src/lib/resume-store.ts`

- [ ] **Step 1: Update `initialResume` in `src/lib/resume-store.ts`**

In `src/lib/resume-store.ts`, import `dummyResumeData` from `./dummy-resume` and replace the hardcoded `initialResume` object.

Modify `src/lib/resume-store.ts` near line 35:
...
```
Make sure to delete the old hardcoded properties from `initialResume`.

- [ ] **Step 2: Run tests to verify the initial state logic remains intact**
...
git commit -m "refactor: update initialResume to use shared dummyResumeData"
```
