...
**Architecture:** Use `@redocly/cli` configured with standard recommended rules to validate schema structure and reference correctness. Port the custom endpoint/schema validation checks into a dedicated Vitest test file `apps/backend/test/openapi.test.ts`.
...
- [ ] **Step 1: Update package.json to include `@redocly/cli` and update the `openapi:lint` script**
  Modify `apps/backend/package.json` to add `"@redocly/cli": "^2.31.6"` to `devDependencies` and update the `openapi:lint` script to run `redocly lint openapi.json`.
...
- [ ] **Step 3: Run the modified lint script to ensure redocly runs (and fails or warns due to lack of config)**
  Run: `pnpm --filter @open-resume/backend openapi:lint` in `/Users/ben/ghq/github.com/Benjaminlooi/resume-builder/.worktrees/job-application-ai-helper`
...
**Files:**
- Create: `apps/backend/redocly.yaml`
...
- [ ] **Step 3: Commit the config file**
  Run:
  ```bash
  git add apps/backend/redocly.yaml
...
**Files:**
- Create: `apps/backend/test/openapi.test.ts`

- [ ] **Step 1: Create a test file with a failing assertion to verify TDD setup**
  Create `apps/backend/test/openapi.test.ts` containing:
  ```typescript
...
- [ ] **Step 2: Run tests to verify the test fails**
  Run: `pnpm --filter @open-resume/backend test` in `/Users/ben/ghq/github.com/Benjaminlooi/resume-builder/.worktrees/job-application-ai-helper`
  Expected: FAIL with "expected undefined to be defined".

- [ ] **Step 3: Replace with the actual contract assertions**
  Update `apps/backend/test/openapi.test.ts` with the complete validation assertions ported from the old script:
  ```typescript
  import { readFileSync } from "node:fs";
  import { resolve } from "node:path";
  import { describe, it, expect } from "vitest";
...
- [ ] **Step 5: Commit the contract tests**
  Run:
  ```bash
  git add apps/backend/test/openapi.test.ts
...
**Files:**
- Delete: `apps/backend/src/validate-openapi.ts`

- [ ] **Step 1: Delete the old custom validation script**
  Run: `rm apps/backend/src/validate-openapi.ts` in `/Users/ben/ghq/github.com/Benjaminlooi/resume-builder/.worktrees/job-application-ai-helper`
...
- [ ] **Step 3: Commit the cleanup**
  Run:
  ```bash
  git add apps/backend/src/validate-openapi.ts
