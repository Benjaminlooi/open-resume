...
describe('settingsStore', () => {
  ...
  it('updates base URL for a provider', () => {
    useSettingsStore.getState().updateBaseUrl('ollama', 'http://localhost:11434/v1');
    expect(useSettingsStore.getState().baseUrls['ollama']).toBe('http://localhost:11434/v1');
  });

  it('updates selected model for a provider', () => {
    useSettingsStore.getState().updateSelectedModel('lmstudio', 'mistral-7b');
    expect(useSettingsStore.getState().selectedModels['lmstudio']).toBe('mistral-7b');
  });
});
