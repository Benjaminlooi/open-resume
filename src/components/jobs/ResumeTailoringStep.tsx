...
import { Loader2, Sparkles, Check, X, AlertTriangle, Info } from "lucide-react";
...
export default function ResumeTailoringStep({ applicationId }: ResumeTailoringStepProps) {
	const {
		jobApplications,
	...
	const { tailoredResume, resumeEditProposals, fitBrief, sourceResumeSnapshot } = application;
	...
	const handleGenerateProposals = async () => {
		if (!fitBrief) {
			setError("Cannot generate proposals: Fit Analysis must be generated first.");
			return;
		}
		...
		try {
			...
			console.error(err);
			setError(err.message || "Failed to generate resume tailoring proposals.");
		} finally {
			setIsGenerating(false);
		}
	};

	const getTargetDescription = (target: any, resume: any) => {
		if (!target) return "Unknown Target";
		if (target.section === "summary") {
			return "Profile Summary";
		}
		if (target.section === "experience") {
			const item = resume?.experience?.find((e: any) => e.id === target.itemId);
		...
		if (target.section === "skills") {
			const item = resume?.skills?.find((s: any) => s.id === target.itemId);
			const category = item?.category || "Skills Category";
			return `Skills: ${category}`;
		}
		if (target.section === "projects") {
			const item = resume?.projects?.find((p: any) => p.id === target.itemId);
			...
			<div className="flex justify-between items-center border-b-2 border-border pb-4">
				<div>
					...
					<p className="text-sm text-muted-foreground mt-1">
						Create a tailored version of your resume by reviewing and approving specific AI proposals.
					</p>
				</div>
			...
			{!tailoredResume ? (
				<div className="border-2 border-border rounded-base p-6 bg-[#FFFBEB] border-[#FEF3C7] shadow-light flex flex-col gap-4">
					<div className="flex gap-3 items-start">
						...
						<div>
							<h3 className="text-lg font-heading mb-1 text-[#B45309]">Start Resume Tailoring</h3>
							<p className="text-sm leading-relaxed text-[#78350F] mb-3">
								Tailoring will create a job-specific copy of your default resume. Your original default resume will remain untouched.
							</p>
							{defaultResumeId ? (
								<p className="text-sm text-[#78350F] font-bold">
									Selected Default Resume: <span className="bg-white/80 px-2 py-0.5 rounded border border-[#FDE68A]">{defaultResumeName}</span>
								</p>
							) : (
								<div className="bg-red-50 text-red-900 border border-red-200 rounded p-2 text-xs font-bold mt-2">
									⚠️ No default resume is currently selected. Please set a default resume on the Resumes page to begin tailoring.
								</div>
							)}
						</div>
					</div>
				...
				<div className="flex flex-col gap-6">
					...
					<div className="bg-[#F0F9FF] border-2 border-[#B9E6FE] rounded-base p-4 flex flex-col md:flex-row md:items-center justify-between gap-3 text-sm">
						<div>
							<span className="font-bold text-[#0369A1]">Tailoring Branch Created:</span>
							<p className="text-muted-foreground mt-0.5">
								Cloned from source: <span className="font-semibold">{application.sourceResumeName || "Default Resume"}</span>
							</p>
						</div>
						{!fitBrief && (
							<div className="text-amber-800 font-bold text-xs bg-amber-50 border border-amber-200 p-2 rounded">
								⚠️ You must generate a Fit Analysis in Step 2 before generating proposals.
							</div>
						)}
					</div>
					...
					{resumeEditProposals.length === 0 ? (
						<div className="border-2 border-dashed border-border rounded-base p-12 bg-white flex flex-col items-center justify-center text-center">
							...
							<h3 className="text-xl font-heading mb-2">No Proposals Generated</h3>
							<p className="text-muted-foreground max-w-md mb-6">
								AI will analyze the gaps between your resume and the job requirements and suggest specific, non-destructive rewrites.
							</p>
							<button
								type="button"
						...
						<div className="flex flex-col gap-6">
							...
							<div className="flex flex-col gap-4">
								<h3 className="text-xl font-heading">AI Recommendations ({resumeEditProposals.length})</h3>
								
								{resumeEditProposals.map((proposal) => {
									const isApplied = proposal.status === "applied" || proposal.status === "approved";
										...
										>
											<div className="flex items-start justify-between gap-4">
												<div>
													<h4 className="font-heading text-lg text-main-foreground">
														{getTargetDescription(proposal.target, tailoredResume)}
													</h4>
													<p className="text-sm font-bold text-muted-foreground mt-1">
														Rationale: <span className="font-normal text-[#082F49]">{proposal.rationale}</span>
													</p>
												</div>
											...
											<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
												<div className="flex flex-col gap-1.5">
													<span className="text-xs font-bold uppercase text-[#9F1239]">Current Content</span>
												...
												<div className="flex flex-col gap-1.5">
													<span className="text-xs font-bold uppercase text-[#14532D]">Proposed Tailoring</span>
											...
											{isPending && (
												<div className="flex justify-end gap-3 border-t border-border pt-3 mt-1">
													<button
														type="button"
														onClick={() => rejectResumeEditProposal(applicationId, proposal.id)}
													...
													<button
														type="button"
														onClick={() => applyResumeEditProposal(applicationId, proposal.id)}
